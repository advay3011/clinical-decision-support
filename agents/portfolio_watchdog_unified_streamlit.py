#!/usr/bin/env python3
"""
Portfolio Watchdog Unified Streamlit App
Combines CLI agent and Financial Advisor agent in one beautiful web interface
"""
...
with st.sidebar:
    ...
    page = st.radio(
        "Choose a page:",
        ["📊 Dashboard", "🔍 Check Ticker", "⚡ Alerts", "⚙️ Configuration", "💬 Financial Advisor", "🤖 CLI Menu"]
    )
...
if page == "📊 Dashboard":
    ...
    st.write("*Powered by AI Financial Advisor Agent*")
    ...
    st.header("🤖 CLI Menu - Interactive Options")
    st.write("Choose an action from the menu below:")
    ...
    with col1:
        ...
        if st.button("1️⃣ Check a Single Ticker", use_container_width=True):
            st.info("Go to 'Check Ticker' page to check a single stock")
        
        if st.button("2️⃣ Check Full Portfolio", use_container_width=True):
            st.info("Go to 'Dashboard' page to see your full portfolio")
        
        if st.button("3️⃣ View Configuration", use_container_width=True):
            st.info("Go to 'Configuration' page to see your settings")
        
        if st.button("4️⃣ Add New Ticker", use_container_width=True):
            st.warning("📝 Edit config.yaml manually or use the CLI version")
        
        if st.button("5️⃣ Remove Ticker", use_container_width=True):
            st.warning("📝 Edit config.yaml manually or use the CLI version")
    
    with col2:
        st.subheader("Advanced Actions")
        
        if st.button("6️⃣ Edit Configuration", use_container_width=True):
            st.warning("📝 Edit config.yaml manually or use the CLI version")
        
        if st.button("7️⃣ Continuous Monitoring", use_container_width=True):
            st.info("💡 Use the CLI version for continuous monitoring")
        
        if st.button("8️⃣ View Alerts", use_container_width=True):
            st.info("Go to 'Alerts' page to see recent alerts")
        
        if st.button("9️⃣ Run Tests", use_container_width=True):
            st.info("Run: `python agents/test_portfolio_comprehensive.py`")
    
    st.divider()
    
    st.subheader("💡 Tips")
    st.write("""
    - **Financial Advisor**: Use the 'Financial Advisor' page for AI-powered analysis
    - **Dashboard**: Get a quick overview of your portfolio
    - **Check Ticker**: Analyze individual stocks in detail
    - **CLI Version**: Run `python agents/portfolio_watchdog_conversational.py` for full CLI experience
    """)
